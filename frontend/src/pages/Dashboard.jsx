...
import {
  ...
  Bell, Plus, FileCheck, FilePen, Award, BarChart3,
          ...
          <div className="bg-[#1a2b4a] rounded-2xl p-8 mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-200 mb-6">
                Gestiona los documentos reglamentarios de tu institución desde aquí.
              </p>
              <button
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div className="bg-white rounded-xl p-5 border border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-gray-400 uppercase tracking-wide">Documentos mes</span>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-gray-400 uppercase tracking-wide">Plantillas</span>
            ...
            <div className={`bg-white rounded-xl p-5 border ${pendingDrafts > 0 ? 'border-yellow-200' : 'border-gray-100'}`}>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-gray-400 uppercase tracking-wide">Borradores IA</span>
                {pendingDrafts > 0 && (
                  <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">Alerta</span>
                )}
              </div>
              <p className="text-3xl font-bold text-gray-900">{pendingDrafts}</p>
              <p className="text-xs text-gray-400 mt-1">Pendientes revisión</p>
            </div>

            <div className="bg-white rounded-xl p-5 border border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-gray-400 uppercase tracking-wide">Estado plan</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-semibold text-gray-800">Documentos Recientes</h3>
                <button
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 font-medium">Sin documentos aún</p>
                  <p className="text-gray-400 text-sm mt-1">Genera tu primer documento reglamentario</p>
                  <button
                    onClick={() => navigate('/documentos/nuevo')}
                ...
                <div className="space-y-3">
                  ...
                  {documents.slice(0, 5).map((doc) => (
                    <div key={doc.id} className="grid grid-cols-4 items-center py-2 hover:bg-gray-50 rounded-lg px-2 -mx-2 transition-colors">
                      <div className="col-span-2 flex items-center gap-2">
                        <FileText size={14} className="text-gray-400 flex-shrink-0" />
                        <div>
                          <p className="text-sm font-medium text-gray-800 truncate">{doc.template_id}</p>
                          <p className="text-xs text-gray-400">{doc.status}</p>
                        </div>
                      </div>
                      <span className={`text-xs px-2 py-1 rounded-full font-medium w-fit ${STATUS_STYLES[doc.status]}`}>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              <h3 className="font-semibold text-gray-800 mb-5">Accesos Directos</h3>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Nueva Acta', icon: FilePen, path: '/documentos/nuevo' },
                  { label: 'Certificado', icon: Award, path: '/documentos/nuevo' },
                  { label: 'Matrícula', icon: ClipboardList, path: '/matriculas' },
                  { label: 'Calificaciones', icon: BarChart3, path: '/documentos/nuevo' },
              ...
              <div className="mt-6 pt-5 border-t border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-gray-500">Documentos usados</span>
                  <span className="text-xs font-medium text-gray-700">{documents.length} / ∞</span>
                </div>
